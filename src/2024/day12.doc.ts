 ...
 * They'd like to set up fences around each region of garden plots, but they can't figure out how much fence
 * they need to order or how much it will cost. They hand you a map (your puzzle input) of the garden plots.
 *
 * Each garden plot grows only a single type of plant and is indicated by a single letter on your map. When
 * multiple garden plots are growing the same type of plant and are touching (horizontally or vertically),
 ...
 * area and perimeter.
 *
 * The area of a region is simply the number of garden plots the region contains. The above map's type A, B,
 * and C plants are each in a region of area 4. The type E plants are in a region of area 3; the type D plants
 ...
 * plots in the region that do not touch another garden plot in the same region. The type A and C plants are each
 * in a region with perimeter 10. The type B and E plants are each in a region with perimeter 8. The lone D plot
 ...
 * OOOOO
 * </pre>
 * The above map contains five regions, one containing all of the O garden plots, and the other four each
 * containing a single X plot.
 *
 * The four X regions each have area 1 and perimeter 4. The region containing 21 type O plants is more complicated;
 * in addition to its outer edge contributing a perimeter of 20, its boundary with each X region contributes an
 ...
 */
export const part1: number = 1304764

/**
 * <h2>Part 2</h2>
 * Fortunately, the Elves are trying to order so much fence that they qualify for a bulk discount!
 *
 * Under the bulk discount, instead of using the perimeter to calculate the price, you need to use the number
 * of sides each region has. Each straight section of fence counts as a side, regardless of how long it is.
 *
 ...
 * EEEEE
 * </pre>
 * The E-shaped region has an area of 17 and 12 sides for a price of 204. Including the two regions full of
 * type X plants, this map has a total price of 236.
 *
 ...
 * (with 4 sides on the outside and 8 more sides on the inside, a total of 12 sides). Be especially careful
 * when counting the fence around regions like the one full of type A plants; in particular, each section of
 * fence has an in-side and an out-side, so the fence does not connect across the middle of the region (where
 * the two B regions touch diagonally). (The Elves would have used the Möbius Fencing Company instead, but their
 ...
 */
export const part2: number = 811148
