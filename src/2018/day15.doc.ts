 ...
 * positions in that round, regardless of the type of unit or whether other units have moved after
 ...
 * squares it could reach in the fewest steps. A step is a single movement to any adjacent
 * (immediately up, down, left, or right) open (.) square. Units cannot move into walls or other
 * units. The unit does this while considering the current positions of units and does not do
 * any prediction about where units will be later. If the unit cannot reach (find an open path to)
 * any of the squares that are in range, it ends its turn. If multiple squares are in range and
 * tied for being reachable in the fewest steps, the square which is first in reading order is
 * chosen. For example:
 * <pre>
 ...
 * <ol>
 *    <li>Each of the Goblins has open, adjacent squares which are in range
 *        (marked with a ? on the map).</li>
 *    <li>Of those squares, four are reachable (marked @); the other two (on the right)
 *        would require moving through a wall or unit to reach.</li>
 *    <li>Three of these reachable squares are nearest, requiring the fewest steps
 *        (only 2) to reach (marked !).</li>
 *    <li>Of those, the square which is first in reading order is chosen (+).</li>
 * </ol>
 * The unit then takes a single step toward the chosen square along the shortest path to
 * that square. If multiple steps would put the unit equally closer to its destination,
 * the unit chooses the step which is first in reading order. (This requires knowing when
 * there is more than one shortest path so that you can consider the first step of each
 ...
 * #######       #######       #######       #######       #######
 * </pre>
 * The Elf sees three squares in range of a target (?), two of which are nearest (!), and
 * so the first in reading order is chosen (+). Under "Distance", each open square is marked
 * with its distance from the destination square; the two squares to which the Elf could move
 * on this turn (down and to the right) are both equally good moves and would leave the Elf 2
 * steps from being in range of the Goblin. Because the step which is first in reading order
 ...
 * The Elf is in range of three targets: the Goblin above it (with 4 hit points), the Goblin
 * to its right (with 2 hit points), and the Goblin below it (also with 2 hit points).
 * Because three targets are in range, the ones with the lowest hit points are selected: the
 * two Goblins with 2 hit points each (one to the right of the Elf and one below the Elf).
 * Of those, the Goblin first in reading order (the one to the right of the Elf) is selected.
 * The selected Goblin's hit points (2) are reduced by the Elf's attack power (3), reducing
 * its hit points to -1, killing it.
 *
 ...
 * of full rounds that were completed (not counting the round in which combat ends) multiplied
 * by the sum of the hit points of all remaining units at the moment combat ends. (Combat only
 * ends when a unit finds no targets during its turn.)
 *
 ...
 * and so combat ends. So, the number of full rounds that were completed is 47, and the sum of the
 * hit points of all remaining units is 200+131+59+200 = 590. From these, the outcome of the
 ...
 */
export const part1: number = 319410
 ...
 */
export const part2: number = 63168
