...
/**
 * <h2>Part 1</h2>
 * As you see the Elves defend their hot chocolate successfully, you
 * go back to falling through time. This is going to become a problem.
 *
 ...
 * to the value contained by register 0 and store the sum in register 3,
 ...
 * it doesn't seem to indicate the number. However, you can monitor
 * the CPU to see the contents of the registers before and after
 * instructions are executed to try to work them out. Each opcode
 * has a number from 0 through 15, but the manual doesn't say which
 * is which. For example, suppose you capture the following sample:
 * <pre>
 * Before: [3, 2, 1, 1]
 * 9 2 1 2
 * After:  [3, 2, 2, 1]
 ...
 *                            register 1 (which has a value of 2) produces 2,
 *                            which matches the value stored in the output
 *                            register, register 2.
 *  - Opcode 9 could be addi: register 2 (which has a value of 1) plus
 *                            value 1 produces 2, which matches the value
 *                            stored in the output register, register 2.
 *  - Opcode 9 could be seti: value 2 matches the value stored in the output
 *                            register, register 2; the number given for B
 *                            is irrelevant.
 *
 ...
 */
export const part2: number[] = [610, 2, 610 , 3]
